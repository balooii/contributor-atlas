#!/usr/bin/env python3
"""
Update contributor-aliases.txt with draft alias suggestions.

The file has two sections:

  1. Active section (top): uncommented Name <id> ... lines. Read by merge.py
     to canonicalize identities. Preserved verbatim across runs of this script.

  2. Draft sections (bottom): commented "# Name <id> ..." lines, grouped by
     confidence tier. Regenerated on every run. To accept a suggestion,
     uncomment its line — on the next run it joins the active section and
     stops being suggested.

Tiers (highest → lowest confidence):

  very-high — gitlab handle ↔ email binding from deterministic signals:
              the gitlab profile's public_email, <id>-<handle>@noreply commit
              emails, or commit emails whose local-part equals a known gitlab
              handle (gated by name agreement).
  high      — same contributor name across ≥2 sources AND ≥1 identifier
              appears in ≥2 sources.
  medium    — same contributor name (under normalization), no shared identifier.
  low       — names match only after aggressive normalization (initials dropped).

Uncommented entries found inside a draft section (user uncommented in place)
are merged into the active section on rewrite and sorted into their place in
the alphabetical block, so they survive regeneration and stay easy to review.
"""
...
_DEFAULT_HEADER = """\
# contributor-aliases.txt — canonical name + every known ID for each contributor.
# Read by merge.py to unify identities across git, GitLab, and Bugzilla before
# writing contributions.csv.
#
# Active section (this block): one line per contributor, mailmap syntax.
#   Name <id> [<id> ...]
# All IDs on a line belong to the same person; order doesn't matter.
# ID types: <real[at]email.com>, <@gitlab-handle>, <#bugzilla-name>.
#
# Draft sections below are regenerated by make_alias_draft.py on every run.
# To accept a suggestion, uncomment its line — next run sorts it into the
...
def main():
    ...
    with open(aliases_path, "w") as f:
        f.write(header_text if header_text.strip() else _DEFAULT_HEADER)
        for line in block:
            f.write(line + "\n")
